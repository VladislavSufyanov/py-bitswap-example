...
async def run(peer_name: str, storage_path: str, kad_port: int, kad_host: str,
              ...
              peers_adr: List[Tuple[str, int]], log_level: str) -> int:
    ...
    async with Network(peer_cid, web_socket_host, web_socket_port, connect_host, connect_port,
                       kad_host, kad_port, peers_adr, log_level=int_log_level) as network:
...
def main(args: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument('--peers-adr', type=str, nargs='*', default=[],
    ...
    peers_adr = list(map(lambda adr: (adr[0], int(adr[1])), [adr.split(':') for adr in parsed_args.peers_adr]))
    return asyncio.get_event_loop().run_until_complete(
        run(parsed_args.peer_name, parsed_args.storage_path, parsed_args.kad_port, parsed_args.kad_host,
            ...
            parsed_args.connect_host, peers_adr, log_level)
    )
